constant = {
	...
	//肖学卫
//	server_path: "http://172.168.10.14:58083",
	//服务器
//	server_path: "http://119.37.198.8:18088/baoma/",
	//穆杰
	server_path: "http://172.168.10.8:58083",
	//邓德保本地
	//	server_path: "http://172.168.10.68:58083",

	commonParams: {},
	imgPre: "http://oeq364mhx.bkt.clouddn.com/",
	filePre: "",
...
Utils = {
	...
	//多个接口同时获取json
	//var obj = {reqMap: [{key: ,url: }],params: {},fn: ,noAsync: error, type: ""} 
	getMultJson: function(obj) {
		if(obj) {
			obj.reqMap.forEach(function(item, index) {
				Utils.getSimpleJson({
					noAsync: obj.noAsync,
					erorr: obj.error,
					type: obj.type,
					...
					params: obj.params,
					fn: function(data) {
						obj.fn(data, item.key);
					}
				});
			});
		} else {
			alert("请Utils.getMultJson配置参数");
		}
	},
	...
	getSimpleJson: function(obj) {
		if(obj) {
				...
				success: function(data) {
					if(urlFlag) {
						...
						if(data.success) {
							...
							var msg = '服务器错误<br>' + data.errorCtx.errorCode + '：' + data.errorCtx.errorMsg;
							layer.open({
								content: msg,
								skin: 'msg',
								time: 1
				...
				error: function(data) {
					if(obj.error && (typeof obj.error == "function")) {
						...
						if(data.responseJSON && data.responseJSON.errorCtx) {
							msg = '服务器错误<br>' + data.responseJSON.errorCtx.errorCode + '：' + data.responseJSON.errorCtx.errorMsg;
						} else {
							msg = "接口报错了，请检查！！！<br>" + this.url;
						}
						layer.open({
							content: msg,
							skin: 'msg',
			...
			alert("请为Utils.getSimpleJson配置参数");
		}
	},
	...
	ajax: function(obj) {
		if(obj) {
			...
			if(obj.type && (obj.type.toUpperCase() == "PUT" || obj.type.toUpperCase() == "DELETE" || obj.type.toUpperCase() == "PATCH")) {
	...
	formValidate: function(obj) {
		var form = obj.form;
		form.valid(function(errList) {
			if(errList.length) {
				var item = errList[0],
					method = item.method,
					ele = $(item.element),
					msg = '';
				var allMsg = ele.attr("data-msg-" + method);
				if(method == "required") {
					if(allMsg) {
						msg = allMsg;
					} else {
						if(ele.attr("placeholder")) {
							msg = ele.attr("placeholder");
						} else {
							msg = "未设置提示语，请注意！！！";
						}
					}
	...
	asyncImg: function(obj) {
		...
		var html = '<form id="lgm_async_demo_form" style="display: none !important;">' +
			'<input type="file" accept="image/*" capture="camera" name="' + inputName + '" ' + multiple + '/>' +
			'</form>';
					...
					Utils.ajaxSubmit({
						...
						fn: function(result) {
							if(obj.fn && (typeof obj.fn == "function") && result) {
								result.imgPath = constant.imgPre + result.key;
								obj.fn(result);
							}
							console.log("图片上传成功");
						}
	...
	//高德地图获取地理定位
	//需引入<script type="text/javascript" src="http://webapi.amap.com/maps?v=1.3&key=您申请的key值"></script>
	//慧聘账号的     key=ae0912fc4e45b6810bcdebc88045602a  其它项目请务必不要乱用key值，涉及法律问题
	getLocationGaoDe: function(fn, error) {
		//lgm_ekoo_hp_gaode_2016这个本是渲染页面地图的id，界面中可以没有这个id
		//但是这里不能为空，可以随便写，如果界面中有此id，则地图会展示在改id对应的标签中
		var map = new AMap.Map('lgm_ekoo_hp_gaode_2016');
		map.plugin('AMap.Geolocation', function() {
			var geolocation = new AMap.Geolocation();
			geolocation.getCurrentPosition(); //调用定位以触发下面的事件
		...
		function onComplete(data) {
			fn && fn({
				addressObj: data.addressComponent,
				adress: data.formattedAddress,
		...
		function onError(data) {
			error && error(data);
			console.log("由于未知原因，定位失败");
		}
	},

	//点击高德地图来获取点击的经纬度，以选择地区
	//需引入<script type="text/javascript" src="http://webapi.amap.com/maps?v=1.3&key=您申请的key值"></script>
	//慧聘账号的     key=ae0912fc4e45b6810bcdebc88045602a  其它项目请务必不要乱用key值，涉及法律问题
	//var obj = {id: , fn: }
	clickGaoDe: function(obj) {
		function gaoDeCreate(data) {
			var map = new AMap.Map(obj.id, {
				resizeEnable: true,
				center: (data.lng ? [data.lng, data.lat] : [116.400244, 39.913164]), //定位失败时,中心定到北京
				zoom: (data.lng ? 12 : 10)
			});
			var marker = null;
		...
		Utils.getLocationGaoDe(gaoDeCreate, gaoDeCreate);
	},
	...
	//上拉加载,下拉刷新
	//var obj = {root: ,downEnable: fasle, downFn: ,upFn: }
	pullLoad: function(obj) {
		return new PullLoad({
			container: obj.root[0],
			scrollBar: obj.root[0],
			down: {
				distance: 50,
	...
	//tab页初始化
	tabInit: function() {
		$(".ekoo-tab-root").each(function(index, ele) {
			$(ele).find(".ekoo-tab-head>*").click(function() {
				$(this).addClass("tab-active").siblings().removeClass("tab-active");
				$(ele).find(".ekoo-tab-body>*").eq($(this).index()).show().siblings().hide();
			}).eq(0).click();
		});
	},
	...
	//剩余时间处理
	//var obj = {fn: }
	timeRemainHandle: function(obj) {
		$(".time-remain").each(function(i, ele) {
			var endTime = $(ele).attr("ctime");
			timeDifference({
				endTime: endTime,
				ele: $(ele)
			});
		...
		function timeDifference(o) {
			var dateDiff = new Date(o.endTime).getTime() - new Date().getTime();
			var leaveMin = dateDiff % (3600 * 1000);
			var hours = Math.floor(dateDiff / (3600 * 1000));
			var minutes = Math.floor(leaveMin / (60 * 1000));
			var seconds = Math.round((leaveMin % (60 * 1000)) / 1000);
			if(hours < 0 || minutes < 0 || seconds < 0) {
				hours = 0;
				minutes = 0;
				seconds = 0;
				obj && obj.fn && (typeof obj.fn == "function") && obj.fn(o.ele);
			}
			o.ele.html((((hours < 10) ? "0" : "") + hours) + "时" + (((minutes < 10) ? "0" : "") + minutes) + "分" + (((seconds < 10) ? "0" : "") + seconds) + "秒");
		}
	},

	//复制功能,需要引入<script src="js/clipboard.min.js"></script>示例如下，
	//<span id="testId">待复制内容</span>
	//<button class="copy-clip" data-clipboard-target="#testId">复制</button>
	copy: function() {
		if(typeof Clipboard == "function") {
			new Clipboard('.copy-clip').on("success", function(e) {
				layer.open({
					content: "复制成功！",
					skin: 'msg',
					time: 1
